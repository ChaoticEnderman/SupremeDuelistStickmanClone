# Orange portal for portal gun weapon
extends GameArea
class_name Projectile11

var flying_time : int = 0

func _init(player: Player, direction: Vector2, position: Vector2):
	super._init(player, direction, position)
	super.add_projectile_data(load("res://resources/projectile11.tres"))
...
func get_other_portal_displacement():
	if player.weapon.orange_portal == null or player.weapon.blue_portal == null:
		return Vector2.ZERO
	return player.weapon.blue_portal.position - player.weapon.orange_portal.position
...
func _on_game_tick(delta: float):
	if flying_time > 0:
		...
		player.weapon.orange_portal = self
	else:
		pass
